...
```bash
nbb test/run_suite.cljs          # Run the suite
nbb test/run_suite.cljs --lint   # ... and lint the same sources
```

The suite is **10 tests / 41 assertions**. That sentence is not decoration:
`test/run_suite.cljs` reads it and refuses (exit 2) any run that comes in
