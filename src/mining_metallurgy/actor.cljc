(ns mining-metallurgy.actor
  "MetallurgyActor — the ISCO-08 3117 mining and metallurgical technician field
  ...
  The unconditional invariant: the MetallurgyAdvisor can never directly commit
  a record or dispatch a robot action the MetallurgyGovernor refuses — every
  commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mining-metallurgy.advisor :as advisor]
            [mining-metallurgy.governor :as governor]
            [mining-metallurgy.store :as store]))

(defn build-graph
  "Build a compiled MetallurgyActor graph. `store` implements
  `mining-metallurgy.store/Store`. `advisor` implements `mining-metallurgy.advisor/Advisor`
